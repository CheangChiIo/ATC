...
for model in "${MODELS[@]}"; do
    ...
    echo "  docker save atc-${model}:${TARGET}-cu128 | gzip > atc-${model}-${TARGET}-cu128.tar.gz"
done
echo ""
echo "Load on the target machine with:"
echo "  gzip -dc atc-<model>-${TARGET}-cu128.tar.gz | docker load"
echo ""
